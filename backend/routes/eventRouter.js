    ...
    try{
        ...
        for(let i=0; i<eventsArray.length; i++){
            if(eventsArray[i]._id == evId){
                ...
                console.log("duration in for loop ",eventsArray[i].evDuration ,eventsArray[i].evDuration['hrs'],eventsArray[i].evDuration['minutes']);
...
eventRoute.get("/getEventsOfSelectedUserAdmin/:selectedUserId", async(req, res)=>{
...
eventRoute.post("/assignEventAdmin", async(req, res)=>{
    ...
    console.log("reqBody", req.body);
    // let {selectedUserId,evName, evType, evDuration, evLocation} = req.body
    let {assignEventToThisUserId, idOfEventToBAssigned, userIdToWhomEventBelongs} = req.body

    console.log("gotten body data", req.body);


    try{
        let finduserToWhomEventBelongs = await User.find({_id : userIdToWhomEventBelongs})
        ...
        let finduserToWhomEventIsToBeAssigned = await User.find({_id : assignEventToThisUserId})
        let userNameToWhomEventIsToBeAssigned = finduserToWhomEventIsToBeAssigned[0].name
        console.log("User to whom event is to be assigned", userNameToWhomEventIsToBeAssigned)

        let eventsArrayOfUserToWhomEventBelongs = await finduserToWhomEventBelongs[0].events
        console.log("eventsArrayOfUserToWhomEventBelongs", eventsArrayOfUserToWhomEventBelongs);
        ...
        for(let i=0; i<eventsArrayOfUserToWhomEventBelongs.length; i++){
            if(eventsArrayOfUserToWhomEventBelongs[i]._id == idOfEventToBAssigned){
                // console.log("duration in for loop ",eventsArray[i].evDuration['hrs'],eventsArray[i].evDuration['minutes']);
                console.log("eventToBAssigned found ",eventsArrayOfUserToWhomEventBelongs[i])
